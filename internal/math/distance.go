...
// VectorSubtract subtracts b from a element-wise
...
// VectorScale multiplies vector by scalar
...
// VectorNormalize normalizes vector to unit length
...
// VectorNormalizeInPlace normalizes vector in-place
func VectorNormalizeInPlace(v []float32) {
	norm := Norm(v)
	if norm == 0 {
		return
	}
